...
const { argv } = yargs
  .usage(
    "Usage: $0 --config-file=[string] --to=[string] --private-key-file-name=[string]"
  )
...
const getNodeDir = (nodeAddress) =>
  `${process.cwd()}/${to}/${prefix(nodeAddress)}`;
...
const saveKeyPair = async ({ privKey, pubKey, nodeAddress }) => {
  ...
  await writeFileAsync(
    `${nodeDir}/${privateKeyFileName}`,
    createFileContent(privKey)
  );
  await writeFileAsync(`${nodeDir}/key.pub`, createFileContent(prefix(pubKey)));
};

const saveGenesisFile = async (genesis) => {
  genesis.extraData = genesis.extraData.replace(
    "<signers>",
    getSignerAddresses()
  );
  await writeFileAsync(
    `${process.cwd()}/${to}/genesis.json`,
