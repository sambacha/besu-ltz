...
if (query) {
  if (besu_private) {
    ...
    if (!contractAddress) {
      console.error(
        'For querying smart contract states, you must pass in the contract address using the "--contract=" argument'
      );
      process.exit(1);
    }
...
function listen() {
  if (ws) {
    ...
    theContract.once("DataStored", (err, event) => {
      if (err) console.error('Error subscribing to "DataStored" events', err);
      else console.log("Event published", event);
    });
  }
}
