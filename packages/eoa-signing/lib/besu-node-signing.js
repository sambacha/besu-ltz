...
class BesuNodeSigningHandler extends NodeSigning {
  ...
  async deployContract(privateFor, privateFrom) {
    ...
    try {
      ...
      let txReceipt = await this.web3.priv.getTransactionReceipt(
        txHash,
        privateFrom
      );
      if (verbose) {
      ...
      console.log(
        `\tSmart contract deployed, ready to take calls at "${txReceipt.contractAddress}"`
      );
    } catch (error) {
  ...
  async queryTransaction(contractAddress, privateFor, privateFrom) {
    ...
    console.log(
      `=> Calling smart contract at "${contractAddress}" for current state value`
    );
    try {
      let res = await rpcInstance.post("", body);
      if (verbose) {
...
module.exports = BesuNodeSigningHandler;
